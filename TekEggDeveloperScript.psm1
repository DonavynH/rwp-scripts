...
function Read-Requirements {
    ...
    $AllReportsPath = ".\application\web\test\build\reports\coverage\requirements", ".\service\stripe\test\build\reports\coverage\requirements", ".\service\user\test\build\reports\coverage\requirements"
    function Open-Requirement {
        param (
            $Path
        )
        Set-FilePath
        $AllFileRequirements = Get-ChildItem $Path
        $InitalJson = @"
        []
"@
        $Output = ConvertFrom-Json -InputObject $InitalJson
        foreach ($Requirement in $AllFileRequirements) {
            $JsonObj = (Get-Content "${path}\${Requirement}" | ConvertFrom-Json)
            $Output += ($jsonObj.requirements)
        }
        return $Output
    }
    $ReportPath = $AllReportsPath[0]
    try {
        Open-Requirement $ReportPath | ConvertTo-Json -Depth 100 -Compress | Out-File -FilePath "${ReportPath}\All.requirements.rpt"  
    }
    catch {
        Write-Error -Message "Requirements not found in ${$ReportPath}" -Category ResourceUnavailable 
    }
}
Set-Alias rdreq Read-Requirements
